...
GetVidConfig PROC
        ...
        int     10h
;;; 10h/1A00h checks for type of graphics card in the system.  The code in
;;; BL is as follows:
;;; 00h    no display
...
;;; 10h/1A00h returns 1Ah in ah if the fuction is unsupported, which probably
;;; means a legacy graphics card or third party whose BIOS doesn't support
;;; this function.


;;; This is a bit of a tangles mess of jumps and compares.  I'm sure that could
...
;;; 10h/12h,BL=10h gets EGA video mode info, it's being used here to check
;;; if the card is really a CGA card.
...
;;; 11h BIOS get equipment list.  Bits 4-5 are the inial video mode.
...
;;; 10h/0Fh gets current video mode
...
;;; The offset is calclated here so you address B000 or B800 properly.


        sub     ax, ax
        mov     es, ax
        mov     al, es:[44Ah]           ; Get number of display cols
;;; I'm not sure why the the 44Ah "video columns on screen" is being used here
;;; when 10h/0Fh just retuend that information
        ...
        jl      exit                    ; No?  Exit
;;; Apparently non-VGA only supports 24 rows?
...
;;; 10h/1130h gets font information, dl is highest row on screen
