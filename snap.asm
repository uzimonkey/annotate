...
;;; Using codepage 437 box drawing characters here, does not render
;;; properly in a modern editor


OldPos  WORD    ?                       ; Original cursor position
Handle  WORD    ?                       ; File handle number
Filez   BYTE    (LEN - 3) DUP(0)        ; ASCIIZ string for file spec
;;; LEN is used here because you need to enter the the filename into the
;;; popup box, any longer and it wouldn't fit.
...
;;; Defines the background attribute byte used when drawing the box.
...
ENDIF
;;; This buffer is used for capturing the screen data, both to save to file
;;; and so the popup box can be un-drawn.
